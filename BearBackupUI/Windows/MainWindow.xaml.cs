...
public partial class MainWindow : FluentWindow
{
    ...
    private void RemoveRecordButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (result == MessageBoxResult.Yes)
        {
            ...
            var selectedRecords = RecordListView.SelectedItems.Cast<RecordViewObject>();
            _dispatchCenter.DispatchEvent(
                new ActionArgs(
                    MainAction.RemoveRecord,
                    (selectedRepo.Record, selectedRecords.Select(r => r.RecordInfo).ToArray())
                ),
                newThread: true);
        }
    }
    ...
    private void RecordListView_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        else if (RecordListView.SelectedItems.Count == 1)
        {
